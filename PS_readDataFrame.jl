...
PR40t = hcat([D.PR40[ (D.rep.==i),:][:,1] for i in 1:Nreps]...)
PR45t = hcat([D.PR45[ (D.rep.==i),:][:,1] for i in 1:Nreps]...)
PR50t = hcat([D.PR50[ (D.rep.==i),:][:,1] for i in 1:Nreps]...)

NR40t = hcat([D.NR40[ (D.rep.==i),:][:,1] for i in 1:Nreps]...)
NR45t = hcat([D.NR45[ (D.rep.==i),:][:,1] for i in 1:Nreps]...)
...
for rep in 1:Nreps
    ...
    sp = Spline1D(R[i,rep], PR40t[i,rep])  
    PR40[:,rep] = sp(Rgrid)

    sp = Spline1D(R[i,rep], PR45t[i,rep])  
    PR45[:,rep] = sp(Rgrid)
    
    sp = Spline1D(R[i,rep], PR50t[i,rep])  
    PR50[:,rep] = sp(Rgrid)

    sp = Spline1D(R[i,rep], NR40t[i,rep])  
    NR40[:,rep] = sp(Rgrid)

    sp = Spline1D(R[i,rep], NR45t[i,rep])  
    NR45[:,rep] = sp(Rgrid)
...
PR40S = sort(PR40, dims = 2)
PR45S = sort(PR45, dims = 2)
PR50S = sort(PR50, dims = 2)

NR40S = sort(NR40, dims = 2)
NR45S = sort(NR45, dims = 2)
