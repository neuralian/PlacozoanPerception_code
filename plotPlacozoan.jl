...
ax1 = Axis(fig1[1,1], 
    ...
    title = "Range Uncertainty of Bayesian Observer",
...
band!(Rgrid,PosteriorEntropy_sorted[:,iQ75] ,PosteriorEntropy_sorted[:,iQ25], color = (BayesFG, bandAlpha) )
#[ lines!(Rgrid, PosteriorEntropy[:, rand(1:Ncols)], color = BayesFG, linewidth = traceW) for _ in 1:Neg]


lines!([25.0, 50.0],[11.0, 11.0], color = BayesFG_comp, linewidth = thickLineW )
text!( "11.0", position = (63.0, 10.8), textsize = 20)
lines!([150.0, 125.0],19.0*[1.0, 1.0], color = BayesFG_comp, linewidth = thickLineW ) 
text!( "19.0", position = (123.0, 18.8), textsize = 20)
...
# band!(Rgrid,KLD_random_sorted[:,iQ75] , KLD_random_sorted[:,iQ25], color = (greenishFG, bandAlpha) )
...
band!(Rgrid,KLD_sample_sorted[:,iQ75] ,KLD_sample_sorted[:,iQ25], color = (BayesFG, bandAlpha) )
...
band!(Rgrid,KLD_particles_sorted[:,iQ75] , KLD_particles_sorted[:,iQ25], color = (ParticleFG, bandAlpha) )

lines!([25.0, 50.0],KLD_random_sorted[end,iQ50]*[1.0, 1.0], color = BayesFG_comp, linewidth = thickLineW ) 
text!( "9.0", position = (58.0, 8.95), textsize = 16)
...
band!(Rgrid, P50_sorted[:, iQ25], P50_sorted[:, iQ75], color = (BayesFG, bandAlpha) )
...
band!(Rgrid, N40_sorted[:, iQ25], N40_sorted[:, iQ75], color = (ParticleFG, bandAlpha))
...
ax3 = Axis(fig3, #xticks = [25 50 100], 
   # yticks = [0 0.25 0.5 0.75 1.0],
    title = "Range Credibility",
    xlabel = "True Range (μm)",
    ylabel = "Inferred Range (μm)",
    xticks = dticks,
    yticks = vec([0 25 50 75 100 125 150 175 200])  )



band!(Rgrid, QP05_sorted[:,iQ50], QP50_sorted[:,iQ50], color = (BayesFG, bandAlpha) )
...
band!(Rgrid, QN05_sorted[:,iQ50], QN50_sorted[:,iQ50], color = (ParticleFG, bandAlpha) )
...
lines!(Rgrid, Rgrid, color = greenishFG, linewidth = thickLineW )
...
band!(Rgrid, QΘ25_sorted[:, iQ50], QΘ75_sorted[:, iQ50], color = (ParticleFG, bandAlpha) )
#ParticleLine4 = [ lines!(Rgrid, QΘ50[:, rand(1:Ncols)], color = ParticleFG, linewidth = traceW) for _ in 1:Neg]

band!(Rgrid, Qψ25_sorted[:, iQ50], Qψ75_sorted[:, iQ50], color = (BayesFG,bandAlpha) )
...
SC = 1.0
band!(Rgrid, MP_sorted[:, iQ25], MP_sorted[:, iQ75], color = (BayesFG, bandAlpha) )
...
band!(Rgrid, SC*MN_sorted[:, iQ25], SC*MN_sorted[:, iQ75], color = (ParticleFG, bandAlpha) )
